"""
src/controllers/settings/api.py
Web API Router for Settings and Credentials management endpoints.
"""
...
from src.instances.config import get_settings, reload_settings
from src.middleware.auth import verify_token
from src.controllers.settings.model import UpdateCredentialsRequest, TestConnectionRequest
from src.services.settings.service import SettingsService
from src.services.scheduler.service import load_scheduler_state, sync_taishin_assets, sync_esun_assets
...
async def save_credentials(body: UpdateCredentialsRequest):
    if body.gemini_api_key is not None:
        SettingsService.update_env_variable("GEMINI_API_KEY", body.gemini_api_key)
        
    if body.taishin_account_id is not None:
        SettingsService.update_env_variable("TAISHIN_ACCOUNT_ID", body.taishin_account_id)
    if body.taishin_api_key is not None:
        SettingsService.update_env_variable("TAISHIN_API_KEY", body.taishin_api_key)
    if body.taishin_api_secret is not None:
        SettingsService.update_env_variable("TAISHIN_API_SECRET", body.taishin_api_secret)
    if body.taishin_cert_password is not None:
        SettingsService.update_env_variable("TAISHIN_CERT_PASSWORD", body.taishin_cert_password)
        
    if body.sinopac_account_id is not None:
        SettingsService.update_env_variable("SINOPAC_ACCOUNT_ID", body.sinopac_account_id)
    if body.sinopac_api_key is not None:
        SettingsService.update_env_variable("SINOPAC_API_KEY", body.sinopac_api_key)
    if body.sinopac_api_secret is not None:
        SettingsService.update_env_variable("SINOPAC_API_SECRET", body.sinopac_api_secret)
    if body.sinopac_cert_password is not None:
        SettingsService.update_env_variable("SINOPAC_CERT_PASSWORD", body.sinopac_cert_password)
