...
| 即時解析進度 | 上傳頁面提供步驟式進度追蹤（傳送、讀取、Gemini 處理、欄位對齊），即時掌握解析進度 |
| API 連線診斷 | 設定頁面提供「測試 API 連線」工具，快速診斷券商 SDK 與 Gemini API 的連線與憑證狀態 |
| 收折式側邊欄 | 導覽列採用極簡 SVG 圖示，支援寬度收折切換，並自動記憶收合狀態以放大報表呈现空間 |
| PDF 報表 | 自動生成精美排版的企業級 PDF 財務報表 |
