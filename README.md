...
* 📈 **證券資產自動估值**：整合台灣主流券商 API（台新、玉山 Fugle）與海外券商（Firstrade），自動更新持股庫存，根據收盤現價即時估算未實現損益。
...
| **分期付款自動扣減** | 支援定期定額分期付款（負債類帳戶）。啟用後，系統每月份會自動從您的負債餘額中扣減此金額，直到餘額歸零。 |
| **AI 助理與 SQL 控制台**| 整合內建的 Gemini AI 助理對話框與開發者模式 SQL 控制台，方便進行即時財務諮詢與底層數據庫直接查詢。詳見 [AI 助理工具與函式說明](docs/ai_assistant_tools.md)。 |
...
### 定期定額分期付款 (Installment Liabilities)
針對手動登錄的負債帳戶（例如：除毛分期、大額消費分期等）：
1. 建立或編輯帳戶時，勾選 **「這是『定期定額分期付款』」**。
2. 輸入 **「每期應繳/扣除金額 (TWD)」**。
3. 啟用後，系統每月份會自動從您的負債餘額中扣減此金額，直到餘額歸零。

### 手動股票庫存 (Manual Stock Holdings)
若您使用的券商目前沒有啟用 API 同步，或是 API 連線有問題：
1. 前往 **「股票庫存」** 頁面。
